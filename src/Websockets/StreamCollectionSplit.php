...
use Countable;
use Iterator;
use Phrity\Net\Stream;
use Phrity\Net\StreamException;
use Phrity\Util\ErrorHandler;

class StreamCollectionSplit implements Countable, Iterator
{
    protected $handler;
    private $streams = [];
    ...
    public function collect()
    {
        ...
        foreach ($this->streams as $key => $stream) {
            if ($stream->isReadable()) {
                $read[$this->keyBase . "-" . $key] = $stream->getResource();
            }
        }
        if (empty($read)) {
            return [];
    ...
    public function ready(?array $changed): self
    {
        if(!$changed){
            return new self($this->keyBase);
        }
        $ready = new self($this->keyBase);
        foreach ($changed as $_key => $resource) {
            [$base, $key] = explode("-", $_key);
            if ($base == $this->keyBase)
    ...
    {
        $readables = new self($this->keyBase);
        foreach ($this->streams as $key => $stream) {
            if ($stream->isReadable()) {
    ...
    {
        $writables = new self($this->keyBase);
        foreach ($this->streams as $key => $stream) {
    ...
    public function waitRead(int $seconds = 60): self
    {
        ...
        if (empty($read)) {
            return new self($this->keyBase); // Nothing to select
        }
